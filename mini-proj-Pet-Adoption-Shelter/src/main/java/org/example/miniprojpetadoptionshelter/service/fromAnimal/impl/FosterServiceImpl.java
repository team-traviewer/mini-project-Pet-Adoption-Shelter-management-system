...
public class FosterServiceImpl implements FosterService {
    ...
    @Override
    public ResponseDto<List<FosterListRes>> getFosterList(
        ...
        boolean hasFilter = fosterUserId != null || status != null || startDate != null || endDate != null;

        List<Foster> fosters = null;

        if (hasFilter) {
            fosters = fosterRepository.searchFosters(fosterUserId, status, startDate, endDate);
        } else {
            fosters = fosterRepository.findAll();
        }

        List<FosterListRes> data = fosters.stream()
